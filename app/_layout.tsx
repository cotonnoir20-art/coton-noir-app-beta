...
function RootLayoutNav() {
  ...
  const isLanding = !inTabs && (pathname === "/" || pathname === "");
  ...
  useEffect(() => {
    ...
    const isRestrictedAuthScreen =
      inAuthGroup && !!authScreen && !publicAuthScreens.has(authScreen);
    if (
      !session &&
      ((!inAuthGroup && !isPublicLegal && !isLanding) || isRestrictedAuthScreen)
    ) {
  ...
  const isPublicLegal =
    ...
    segments[0] === "legal";
  const showTabBar =
    !!session && !loading && !inAuthGroup && !isPublicLegal && !isLanding;
...
export default function RootLayout() {
  ...
  useEffect(() => {
    if (error && __DEV__) {
      console.warn(
        "[fonts] Chargement partiel des polices — l’app continue.",
        error,
      );
    }
  }, [error]);
